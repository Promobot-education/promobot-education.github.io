...
**RRStudio** - десктопное приложение предназначенное для управления Promobot Rooky и Promobot Robox с помощью
алгоритмов Blockly.
...
RRStudio подключается к Rosbridge по веб-сокету. Для этого вам нужно ввести ip-адрес [сервера](/RRStudio#rosserver).  
...
RRStudio подключается к Robox с помощью USB-соединения. Для этого подключите Robox к USB-порту компьютера и выберите
...
сервопривод продолжает работать после закрытия RRStudio. Поэтому не забывайте отключать Robox после работы.
